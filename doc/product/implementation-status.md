...
| X post ingestion | Implemented | Bounded 20-post recent-window parsing for originals, replies, reposts, and quoted context. |
...
| Durable queue, retries, and deduplication | Implemented | SQLite-backed oldest-first queue, restart recovery, and ID checkpoints prevent silent loss and unnecessary reclassification. |
...
- Discovery and classification are decoupled, but a large backlog can still take time to drain because local inference is intentionally serialized.
