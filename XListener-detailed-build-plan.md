...
7. Let the user submit a missed post URL later, then ask for a 1-10 score before fetching and retaining that post.
...
The classifier assigns one to eight model-generated tags to every processed item. Feedback updates learned affinity for those normalized tags automatically. Future classification prompts receive the learned tag profile and representative high/low-rated examples.
...
Ignored posts are deliberately not archived. If the user later notices an important missed post, they send the bot its X URL by itself, for example:

```text
https://x.com/.../status/...
```

The bot validates the URL without fetching it, then asks for a usefulness rating through inline buttons from 1 through 10. Only after the user selects a rating does it fetch the post through the authenticated X session, resolve its reply/quote/repost context, classify and tag it, and store the complete record as a `user_submitted_missed` item. The score is saved as feedback and immediately updates learned tag affinity. A score above 5 is treated as evidence of a false negative, but the system accepts and retains all valid scores from 1 through 10.
...
Tags are generated by the model rather than chosen from a fixed vocabulary. The classifier returns one to eight concise topic or product labels. The application normalizes them to lowercase underscore identifiers and removes exact duplicates before validation. Prompt guidance discourages near-duplicate labels, while feedback affinity operates on the normalized tags actually produced.
...
CREATE TABLE feedback (
    ...
    source TEXT NOT NULL, -- inline_button, missed_post
...
- `user_submitted_missed`: the user supplied a post URL and then rated it after the normal listener ignored it.
...
2. Plain text containing one X status URL: validate and canonicalize the URL without fetching it, create an expiring pending request, and ask for a 1-10 rating. When the rating callback arrives, fetch that status plus bounded nested context, classify it, save it as `user_submitted_missed`, append the rating with source `missed_post`, update tag affinity, and send an explicit success or failure confirmation.
...
- Mock Telegram inbound updates: inline rating callback, malformed link, duplicate update, link-only missed-post submission, and rating callback.
...
16. Sending a valid X status link causes a rating prompt; selecting a rating then fetches only that URL, classifies it, retains it as `user_submitted_missed`, and updates learning; previously ignored posts remain unarchived.
...
Add model-generated tag-affinity updates, learned-profile snapshots, rated examples in prompts, and link-only missed-post intake with a rating prompt.
...
| Relevant post is initially ignored | Missed alert | Do not archive every ignored post; provide explicit link-only recovery with a rating prompt, reclassify the submitted post, retain it, and update tag affinity from the user's rating. |
...
- Allow Telegram inline ratings from 1 (irrelevant) to 10 (very useful), and allow link-only submissions followed by a rating prompt to recover missed posts.
